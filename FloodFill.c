...
void saveArrayToFile(int arr[MAZE_SIZE][MAZE_SIZE], const char* filename) {
    FILE* file = fopen(filename, "wb");
    if (file == NULL) {
        perror("Error opening file");
        return;
    }
    fwrite(arr, sizeof(int), MAZE_SIZE * MAZE_SIZE, file);
    fclose(file);
}

void loadArrayFromFile(int arr[MAZE_SIZE][MAZE_SIZE], const char* filename) {
    FILE* file = fopen(filename, "rb");
    if (file == NULL) {
        perror("Error opening file");
        return;
    }
    fread(arr, sizeof(int), MAZE_SIZE * MAZE_SIZE, file);
    fclose(file);
}
void saveCharArrayToFile(char arr[MAZE_SIZE][MAZE_SIZE], const char* filename) {
    FILE* file = fopen(filename, "wb");
    if (file == NULL) {
    ...
    fwrite(arr, sizeof(char), MAZE_SIZE * MAZE_SIZE, file);
    fclose(file);
}
void loadCharArrayFromFile(char arr[MAZE_SIZE][MAZE_SIZE], const char* filename) {
    FILE* file = fopen(filename, "rb");
    if (file == NULL) {
    ...
    fread(arr, sizeof(char), MAZE_SIZE * MAZE_SIZE, file);
    fclose(file);
}
...
int getMinDirection() {
    ...
    char* distStr = intToString(maze[x][y].distance);
    API_setText(x, y, distStr);
    return dir;
}
...
int main(int argc, char* argv[]) {
    ...
    int distArr[MAZE_SIZE][MAZE_SIZE];
    char wallsArr[MAZE_SIZE][MAZE_SIZE];
